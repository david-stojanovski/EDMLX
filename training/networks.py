...
class Conv2d(torch.nn.Module):
    def __init__(self,
                 ...
                 resample_filter=None, fused_resample=False, init_mode='kaiming_normal', init_weight=1, init_bias=0,
                 ):
...
class SPADE(torch.nn.Module):
    def __init__(self, norm_nc, label_nc, eps=1e-5):
        ...
        self.norm = GroupNorm(num_groups=32, num_channels=norm_nc)

        self.eps = eps
        nhidden = 128
        self.mlp_shared = torch.nn.Sequential(
            Conv2d(in_channels=label_nc, out_channels=nhidden, kernel=3),
            torch.nn.ReLU()
        )
        self.mlp_gamma = Conv2d(in_channels=nhidden, out_channels=norm_nc, kernel=3)
        self.mlp_beta = Conv2d(in_channels=nhidden, out_channels=norm_nc, kernel=3)

    def forward(self, x, segmap=None):
        x = self.norm(x)

        if segmap is None:
            segmap = torch.ones((1, 10, 256, 256), dtype=x.dtype, device=x.device)
...
class UNetBlock(torch.nn.Module):
    def __init__(self,
                 in_channels, out_channels, emb_channels, label_nc=0, up=False, down=False, attention=False,
                 num_heads=None, channels_per_head=64, dropout=0, skip_scale=1, eps=1e-5,
                 resample_filter=None, resample_proj=False, adaptive_scale=True,
                 init=None, init_zero=None, init_attn=None, semantic_cond=False,
                 ):
        super().__init__()
        if resample_filter is None:
            resample_filter = [1, 1]
        ...
        norm_func = self.get_norm_func(self)

        if self.semantic_cond:
            self.norm0 = norm_func(label_nc=self.label_nc, norm_nc=self.in_channels)
        else:
            self.norm0 = norm_func(num_channels=in_channels, eps=eps)
        ...
        if self.semantic_cond:
            self.norm1 = norm_func(label_nc=self.label_nc, norm_nc=self.out_channels)

        else:
            self.norm1 = norm_func(num_channels=out_channels, eps=eps)
        ...
        if self.num_heads:
            if self.semantic_cond:
                self.norm2 = norm_func(label_nc=self.label_nc, norm_nc=self.out_channels)
            else:
                self.norm2 = norm_func(num_channels=out_channels, eps=eps)
    ...
    def forward(self, x, emb, segment=None):
        orig = x
        if segment is not None:
            x = self.conv0(silu(self.norm0(x, segment)))
        else:
        ...
        if self.adaptive_scale:
            ...
            if segment is not None:
                x = silu(torch.addcmul(shift, self.norm1(x, segment), scale + 1))
            else:
            ...
            if segment is not None:
                x = silu(self.norm1(x.add_(params), segment))
            else:
        ...
        if self.num_heads:
            ...
            if segment is not None:
                q, k, v = self.qkv(self.norm2(x, segment)).reshape(x.shape[0] * self.num_heads,
...
class SongUNet(torch.nn.Module):
    def __init__(self,
                 ...
                 semantic_cond=False,  # Semantic conditioning
                 semantic_label_nc=0,  # Number of semantic labels, 0 = unconditional.
                 ...
                 resample_filter=None,  # Resampling filter: [1,1] for DDPM++, [1,3,3,1] for NCSN++.
                 ):
        ...
        self.map_label = Linear(in_features=label_dim,
                                ...
                                **init) if label_dim else None
        self.map_augment = Linear(in_features=augment_dim,
                                  out_features=noise_channels,
                                  bias=False,
                                  **init) if augment_dim else None
        self.map_layer0 = Linear(in_features=noise_channels,
    ...
    def forward(self, x, noise_labels, class_labels, semantic_labels, semantic_cond, augment_labels=None):
        # Mapping.
        ...
        for name, block in self.enc.items():
            if 'aux_down' in name:
                ...
                x = block(x, emb) if block._orig_class_name is UNetBlock._orig_class_name else block(x)
                skips.append(x)
        ...
        for name, block in self.dec.items():
            if 'aux_up' in name:
                ...
                if block._orig_class_name == UNetBlock._orig_class_name:
                    x = block(x, emb, semantic_labels)
                else:
                    x = block(x, emb)
        return aux
...
class DhariwalUNet(torch.nn.Module):
    def __init__(self,
                 ...
                 out_channels,  # Number of color channels at output.
                 label_dim=0,  # Number of class labels, 0 = unconditional.
                 semantic_cond=False,  # Use semantic conditioning.
                 semantic_label_nc=0,  # Number of semantic labels, 0 = unconditional.
                 augment_dim=0,  # Augmentation label dimensionality, 0 = no augmentation.
...
class VPPrecond(torch.nn.Module):
    ...
    def forward(self, x, sigma, class_labels=None, semantic_labels=None, semantic_cond='none', force_fp32=False,
                **model_kwargs):
