...
def scale_to_0_255(x):
    return (x + 1) * 127.5


@persistence.persistent_class
class AugmentPipeV2:
    # simple augmentation pipeline used in EDMLX paper
    ...
    def __call__(self, images, labelmaps=None):
        images = scale_to_0_255(
            images)  # idk why but the transforms below don't work properly when using range of [0,1]
        train_transforms = v2.Compose(
            [
                v2.RandomVerticalFlip(),
                v2.RandomHorizontalFlip(),
                v2.RandomRotation(degrees=10., fill=0),
                v2.RandomAffine(degrees=0, translate=(0.1, 0.1), scale=(1, 1),
                                interpolation=v2.InterpolationMode.BILINEAR, fill=0),
                v2.RandomAffine(degrees=0, translate=(0, 0), scale=(0.8, 1.2),
                                interpolation=v2.InterpolationMode.BILINEAR, fill=0),
            ])

        labelmaps = torchvision.tv_tensors.Mask(labelmaps)

        images, labelmaps = train_transforms(images, labelmaps)
        images = (images / 127.5) - 1

        batchsize, _, _, _ = images.shape
        device = images.device
        labels = [torch.zeros([images.shape[0], 0], device=device)]
        labels = torch.cat([x.to(torch.float32).reshape(batchsize, -1) for x in labels], dim=1)
        return images, labels, labelmaps
...
class AugmentPipe:
    def __init__(self, p=1,
                 ...
                 scale=0, rotate_frac=0, aniso=0, translate_frac=0, scale_std=0.2, rotate_frac_max=1, aniso_std=0.2,
                 aniso_rotate_prob=0.5, translate_frac_std=0.125,
        ...
        self.aniso_rotate_prob = float(
            aniso_rotate_prob)  # Probability of doing anisotropic scaling w.r.t. rotated coordinate frame.
        self.translate_frac_std = float(
            translate_frac_std)  # Standard deviation of frational translation, relative to image dimensions.
    ...
    def __call__(self, images):
        ...
        if self.translate_int > 0:
            ...
            w = torch.where(torch.rand([1, N, 1, 1, 1], device=device) < self.translate_int * self.p, w,
                            torch.zeros_like(w))
        ...
        if self.translate_frac > 0:
            ...
            G_inv = G_inv @ translate2d_inv(w[0].mul(W * self.translate_frac_std),
        ...
        if G_inv is not I_3:
            ...
            # Upsample.
            conv_weight = misc.constant(Hz[None, None, ::-1], dtype=images.dtype, device=images.device).tile(
                [images.shape[1], 1, 1])
            conv_pad = (len(Hz) + 1) // 2
            images = torch.stack([images, torch.zeros_like(images)], dim=4).reshape(N, C, images.shape[2], -1)[:, :, :,
                     :-1]
            images = torch.nn.functional.conv2d(images, conv_weight.unsqueeze(2), groups=images.shape[1],
                                                padding=[0, conv_pad])
            images = torch.stack([images, torch.zeros_like(images)], dim=3).reshape(N, C, -1, images.shape[3])[:, :,
                     :-1, :]
            images = torch.nn.functional.conv2d(images, conv_weight.unsqueeze(3), groups=images.shape[1],
                                                padding=[conv_pad, 0])
            ...
            images = torch.nn.functional.grid_sample(images, grid, mode='bilinear', padding_mode='zeros',
                                                     align_corners=False)

            # Downsample and crop.
            conv_weight = misc.constant(Hz[None, None, :], dtype=images.dtype, device=images.device).tile(
                [images.shape[1], 1, 1])
            conv_pad = (len(Hz) - 1) // 2
            images = torch.nn.functional.conv2d(images, conv_weight.unsqueeze(2), groups=images.shape[1], stride=[1, 2],
                                                padding=[0, conv_pad])[:, :, :, Hz_pad: -Hz_pad]
            images = torch.nn.functional.conv2d(images, conv_weight.unsqueeze(3), groups=images.shape[1], stride=[2, 1],
                                                padding=[conv_pad, 0])[:, :, Hz_pad: -Hz_pad, :]
